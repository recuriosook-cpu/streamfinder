...
-- Safe to run multiple times (IF NOT EXISTS / OR REPLACE / IF EXISTS)
-- ================================================================
...
-- Unique index on username (sparse: NULL values are excluded)
CREATE UNIQUE INDEX IF NOT EXISTS profiles_username_unique
  ON profiles(username) WHERE username IS NOT NULL;

-- Allow anyone to read profiles (required for /usuario/[username] and search)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'profiles' AND policyname = 'Public profiles are readable'
  ) THEN
...
-- Backfill: assign username to existing users that don't have one yet
-- (UUID prefix ensures uniqueness across the board)
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='review_likes' AND policyname='Review likes are publicly readable') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='follows' AND policyname='Follows are publicly readable') THEN
