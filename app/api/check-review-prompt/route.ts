...
export async function GET() {
  ...
  if (!user) return NextResponse.json({ shouldShow: false, reason: 'not_logged_in' })
  ...
  if (!profile) return NextResponse.json({ shouldShow: false, reason: 'no_profile' })

  // Condition 6: already confirmed review — never show again
  if (profile.review_prompt_status === 'reviewed') {
    return NextResponse.json({ shouldShow: false, reason: 'already_reviewed' })
  }

  // Condition 4: permanently dismissed
  if (profile.review_prompt_status === 'declined') {
    return NextResponse.json({ shouldShow: false, reason: 'declined' })
  }

  // Condition 5: "later" — respect 7-day cooldown
  if (profile.review_prompt_status === 'later' && profile.review_prompt_last_shown) {
    ...
    if (daysSince < 7) {
      return NextResponse.json({ shouldShow: false, reason: 'cooldown' })
    }
  }

  // Condition 2: account must be at least 3 days old
  const daysSinceSignup = (Date.now() - new Date(user.created_at).getTime()) / 86_400_000
  if (daysSinceSignup < 3) {
    return NextResponse.json({ shouldShow: false, reason: 'too_new' })
  }

  // Condition 3: at least one engagement threshold reached
  ...
  if (!hasEnoughActivity) {
    return NextResponse.json({ shouldShow: false, reason: 'not_enough_activity' })
  }

  return NextResponse.json({ shouldShow: true, reason: 'all_conditions_met' })
}
